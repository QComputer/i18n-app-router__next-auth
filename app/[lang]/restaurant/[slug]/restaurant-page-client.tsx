...
import { MapPin, Phone, Mail, Globe, Store, Users, Clock, ShoppingCart, Plus, Minus, Trash2, Menu, X, ChevronDown, Edit, ArrowRight } from "lucide-react"
...
export function RestaurantPageClient({ organization, dictionary, lang, isOwner, followerCount }: {
  organization: RestaurantOrg
    ...
    <div dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="relative">
        <div className="h-64 md:h-80 w-full bg-muted relative overflow-hidden">
          {organization.coverImage ? <Image src={organization.coverImage} alt={organization.name} fill className="object-cover" priority /> : <div className="absolute inset-0 bg-gradient-to-br from-orange-100 via-red-50 to-pink-100" />}
        ...
        <div className="container mx-auto px-4 -mt-20 relative z-10">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-6">
            <div className="h-32 w-32 border-4 border-background shadow-lg rounded-xl overflow-hidden bg-background">
              {organization.logo || organization.avatarImage ? <Image src={(organization.logo || organization.avatarImage) as string} alt={organization.name} fill className="object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-orange-100"><Store className="h-16 w-16 text-orange-500" /></div>}
            </div>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Menu */}
        <section>
          <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><Menu className="h-6 w-6" />{lang === "fa" ? "منوی رستوران" : "Restaurant Menu"}</h2>
                        ...
                        return (
                          <div key={p.id} className={`border rounded-lg overflow-hidden ${!p.isActive ? "opacity-60" : ""}`}>
                            <div className="aspect-square relative bg-muted">
                              {p.image ? <Image src={p.image} alt={p.name} fill className="object-cover" /> : <div className="absolute inset-0 flex items-center justify-center"><Store className="h-12 w-12 text-muted-foreground" /></div>}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {cart.map(item => (
                  <div key={item.product.id} className="flex gap-4 p-3 border rounded-lg">
                    <div className="w-20 h-20 bg-muted rounded-lg overflow-hidden">{item.product.image ? <Image src={item.product.image} alt={item.product.name} width={80} height={80} className="object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Store className="h-8 w-8 text-muted-foreground" /></div>}</div>
