...
export default function UserCreateForm({ dict, lang }: UserCreateFormProps) {
  const [formData, setFormData] = useState({
    username: "",
    email: "",
    name: "",
    phone: "",
    role: "CLIENT" as 'CLIENT' | 'STAFF' | 'ADMIN' | 'OTHER',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!password) {
      setError(dict.admin?.users?.passwordRequired || "Password is required")
      return
    }
    ...
    if (password.length < 8) {
      setError(dict.auth?.passwordTooShort || "Password must be at least 8 characters")
      return
    }
    ...
    try {
      ...
      setSuccess(dict.admin?.users?.saveSuccess || "User created successfully")
      setFormData({
        username: "",
        email: "",
        name: "",
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {formData.role === 'STAFF' && (
        <div>
          <label htmlFor="hierarchy" className="block text-sm font-medium text-foreground mb-1">
            {dict.admin?.users?.hierarchy || "Hierarchy"}
          </label>
          <select
            id="hierarchy"
            name="hierarchy"
      ...
      <div>
        <label htmlFor="confirmPassword" className="block text-sm font-medium text-foreground mb-1">
          {dict.admin?.users?.confirmPassword || "Confirm Password"}
        </label>
        <input
          type="password"
          id="confirmPassword"
